...
def parse_args():
    parser = argparse.ArgumentParser(description="link prediction baseline--ChebNet")
...
def main(args):
    ...
    for i in range(10):
        ...
        model = Cheb_Link(x.size(-1), args.num_class_link, filter_num=args.num_filter, K=args.K, dropout=args.dropout).to(device)
