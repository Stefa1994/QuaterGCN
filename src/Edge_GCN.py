...
def parse_args():
    parser = argparse.ArgumentParser(description="link prediction baseline--GCN")
    ...
    parser.add_argument('--method_name', type=str, default='GCN', help='method name')
...
def main(args):
    ...
    for i in range(10):
        ...
        model = GCN_Link(x.size(-1), args.num_class_link, filter_num=args.num_filter, dropout=args.dropout).to(device)
...
if __name__ == "__main__":
    ...
    if args.debug:
        args.epochs = 1
    if args.to_undirected: 
        save_name = args.method_name + 'lr' + str(int(args.lr*1000)) + 'num_filters' + str(int(args.num_filter))+ 'task_' + args.task + '_undirected' + '_noisy' +  str(args.noisy)
    else:
