...
def parse_args():
    parser = argparse.ArgumentParser(description="link prediction baseline--APPNP")
    ...
    parser.add_argument('--K', type=int, default=1, help='APPNP propogate steps')
...
def main(args):
    ...
    for i in range(10):
        ...
        model = APPNP_Link(x.size(-1), args.num_class_link, filter_num=args.num_filter, alpha = args.alpha, dropout=args.dropout, K=args.K).to(device)
        ...
        for epoch in range(args.epochs):
            ...
            model.train()
            out = model(x, edges, edge_weight, train_index)
            ...
            train_loss, train_acc = 0.0, 0.0
            model.eval()
            out = model(x, edges, edge_weight, val_index)
        ...
        out = model(x, edges, edge_weight, test_index)
        pred_label = out.max(dim = 1)[1]
    ...
    #        model.load_state_dict(torch.load(log_path + '/model'+str(i)+'.t7'))
    #        model.eval()
    #        out_val = model(x, edges, edge_weight, val_index)
    #        out_test = model(x, edges, edge_weight,  test_index)
...
if __name__ == "__main__":
    ...
    if args.to_undirected: 
        save_name = args.method_name + 'lr' + str(int(args.lr*1000)) + 'num_filters' + str(int(args.num_filter)) + 'alpha' + str(int(100*args.alpha)) + 'K' + str(int(args.K)) + 'task_' + args.task  + '_undirected' + '_noisy' +  str(args.noisy)
    else:
        save_name = args.method_name + 'lr' + str(int(args.lr*1000)) + 'num_filters' + str(int(args.num_filter)) + 'alpha' + str(int(100*args.alpha)) + 'K' + str(int(args.K)) + 'task_' + args.task + '_noisy' +  str(args.noisy)
