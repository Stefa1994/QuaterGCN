...
from layer.geometric_baselines import GATModel
...
def parse_args():
    parser = argparse.ArgumentParser(description="baseline--graph attention.")
...
def main(args):
    ...
    for split in range(splits):
        log_str_full = ''
        graphmodel = GATModel(data.x.size(-1), num_classes, heads=args.heads, filter_num=args.num_filter,
...
if __name__ == "__main__":
    ...
    if os.path.isdir(dir_name) == False:
        try:
            ...
            print('Folder exists!')
    save_name = args.method_name + 'lr' + str(int(args.lr*1000)) + 'num_filters' + str(int(args.num_filter)) + 'tud' + str(args.to_undirected) + 'heads' + str(int(args.heads)) + 'layer' + str(int(args.layer))
