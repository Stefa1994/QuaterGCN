...
def parse_args():
    parser = argparse.ArgumentParser(description="baseline--DGCN")
...
def main(args):
    ...
    data.edge_index, edge_weight, edge_in, in_weight, edge_out, out_weight = F_in_out(data.edge_index, data.y.size(-1), data.edge_weight)
    ...
    data = data.to(device)
    edge_in, in_weight, edge_out, out_weight, edge_weight = edge_in.to(device), in_weight.to(device), edge_out.to(device), out_weight.to(device), edge_weight.to(device)
    ...
    for split in range(splits):
        ...
        for epoch in range(args.epochs):
            ...
            model.train()
            out = model(data.x, data.edge_index, edge_in, in_weight, edge_out, out_weight, edge_weight)
        ...
        model.eval()
        preds = model(data.x, data.edge_index, edge_in, in_weight, edge_out, out_weight, edge_weight)
