...
if args.dataset in ['bitcoin_alpha', 'bitcoin_otc', 'slashdot', 'epinions']:
    ...
    data = read_edge_list(path = f"./data/wikirfa/edges.csv")
...
for split in list(link_data.keys()):
    ...
    if args.method == 'SGCN':
        ...
        edge_index, norm_real, norm_imag = laplacian.process_magnetic_laplacian(edge_index=edge_index, gcn=True, net_flow=False, x_real=X_real, edge_weight=edge_weight, \
        ...
        model = SigMaNet_link_prediction_one_laplacian(K=1, num_features=num_input_feat, hidden=args.hidden, label_dim=args.num_classes,
                            i_complex = False,  layer=args.num_layers, follow_math=False, gcn =True, net_flow=False, unwind = True, edge_index=edge_index,\
