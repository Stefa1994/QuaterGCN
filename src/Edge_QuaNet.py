'''
Quaternion Laplacian con modifica del numero delle iterazioni (3) anziché 10
'''
...
def main(args):
    ...
    #print('-------------------------------------------------------')
    #print('Sono pronto a partire con il training')
    for i in range(10):
        log_str_full = ''

        ########################################
        # get quaternion laplacian matrix
        ########################################
        ...
        model = QuaNet_link_prediction_one_laplacian(K=args.K, num_features=2, hidden=args.num_filter, label_dim=args.num_class_link,
        ...
        for epoch in range(args.epochs):
            ...
            out = model(X_real, X_img_i, X_img_j, X_img_k, train_index)
            ...
            train_loss, train_acc = 0.0, 0.0
            model.eval()
            out = model(X_real, X_img_i, X_img_j, X_img_k,  val_index)
        ...
        out = model(X_real, X_img_i, X_img_j, X_img_k, test_index)
        pred_label = out.max(dim = 1)[1]
...
if __name__ == "__main__":
    ...
    save_name = args.method_name + 'lr' + str(int(args.lr*1000)) + 'num_filters' + str(int(args.num_filter)) + '_quaternion_weights' + str(args.qua_weights)  + '_quaternion_bias' + str(args.qua_bias) + 'task' + args.task + 'layers' + str(args.layer) + '_noisy' +  str(args.noisy)
